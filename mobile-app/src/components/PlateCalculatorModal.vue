  ...
  <Teleport to="body">
    <div v-if="visible" class="plate-backdrop" @click.self="emit('close')">
      <div class="plate-modal">
        ...
        <div class="bar-row">
          <button
            v-for="bar in bars" :key="bar.kg + '-' + bar.label"
            class="bar-btn"
        ...
        <div class="bar-visual">
          <div class="sleeve left">
            <div
              v-for="(p, i) in expandedPlates.slice().reverse()" :key="'l'+i"
              class="plate"
              :style="plateStyle(p)"
            />
          </div>
          ...
          <div class="sleeve right">
            <div
              v-for="(p, i) in expandedPlates" :key="'r'+i"
              class="plate"
              :style="plateStyle(p)"
            />
          </div>
        ...
        <div class="plate-list">
          <div v-if="!platesPerSide.length && remainder < 0.01" class="plate-empty">Bar only</div>
          <div v-for="item in platesPerSide" :key="item.kg" class="plate-item">
            <span class="plate-badge" :style="plateStyle(item.kg)">{{ plateLabelShort(item.kg) }}</span>
...
watch(() => props.weightKg, v => { targetKg.value = v })
...
interface PlateItem { kg: number; count: number }
...
const PLATE_COLORS: Record<number, string> = {
  25: '#E53935', 20: '#1565C0', 15: '#F9A825', 10: '#2E7D32',
  5: '#fff', 2.5: '#666', 1.25: '#333', 0.5: '#222', 0.25: '#1A1A1A',
}
...
function plateStyle(kg: number): Record<string, string> {
  ...
  return {
    height: `${h}px`,
    background: PLATE_COLORS[kg] ?? '#444',
...
.plate-title {
  ...
  font-weight: 800; color: #F0F0F0; letter-spacing: 0.1em;
}
.plate-close {
  background: none; border: none; color: #555; font-size: 1rem;
  cursor: pointer; padding: 0.25rem;
}
.plate-close:active { color: #FF4D00; }
...
.w-btn:active { background: #2A2A2A; color: #FF4D00; }
.weight-display { text-align: center; min-width: 80px; }
.weight-val { font-family: 'Barlow Condensed', sans-serif; font-size: 2.5rem; font-weight: 900; color: #FF4D00; }
.weight-unit { font-family: 'Barlow Condensed', sans-serif; font-size: 1rem; color: #555; margin-left: 4px; }
...
.bar-btn {
  ...
  padding: 0.35rem 0.75rem; cursor: pointer; letter-spacing: 0.05em;
  transition: border-color 0.15s, color 0.15s;
}
.bar-btn.active { border-color: #FF4D00; color: #FF4D00; }

/* Visual bar */
.bar-visual {
  display: flex; align-items: center; justify-content: center;
  gap: 0; height: 64px; margin-bottom: 1.25rem;
  overflow: hidden;
}
.sleeve {
...
.sleeve.right { flex-direction: row; }
.plate {
  width: 10px; border-radius: 2px;
}
.bar-tube {
  width: 20px; height: 8px; background: #888; border-radius: 1px;
}
.collar { width: 8px; height: 14px; background: #aaa; border-radius: 1px; }
.bar-center {
  width: 100px; height: 8px; background: #888; border-radius: 1px;
  flex-shrink: 0;
}
...
.plate-badge {
  width: 56px; height: 28px; border-radius: 2px;
  display: flex; align-items: center; justify-content: center;
  font-family: 'Barlow Condensed', sans-serif; font-size: 0.85rem; font-weight: 800;
  color: #F0F0F0; text-shadow: 0 1px 2px rgba(0,0,0,0.6);
  flex-shrink: 0;
}
.plate-count {
  font-family: 'Barlow Condensed', sans-serif; font-size: 1rem;
