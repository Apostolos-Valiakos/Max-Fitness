<template>
  <div>
    <div class="window-row">
      <button v-for="w in WINDOWS" :key="w.days" class="window-btn" :class="{ active: days === w.days }" @click="setWindow(w.days)">{{ w.label }}</button>
    </div>
...
const chartOptions = {
  ...
  scales: {
    ...
    y: { ticks: { color: '#888', font: { size: 10 } }, grid: { color: 'transparent' } },
  },
}
...
function setWindow(d: number) { days.value = d }

watch(days, load, { immediate: true })
</script>

<style scoped>
.window-row { display: flex; gap: 0.35rem; margin-bottom: 0.5rem; }
