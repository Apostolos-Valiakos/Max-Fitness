<template>
  <nav class="bottom-nav" :class="{ 'has-trainer': isTrainerOrAdmin }">
    ...
    <router-link v-if="isTrainerOrAdmin" to="/trainer" class="nav-item" active-class="active">
...
.bottom-nav {
  ...
  align-items: center;
  background: #111111; border-top: 1px solid #1A1A1A;
...
.nav-item {
  ...
  color: #555; text-decoration: none; font-family: 'DM Sans', sans-serif;
  font-size: 0.65rem; font-weight: 500; letter-spacing: 0.02em;
  transition: color 0.2s;
}
.nav-item i { font-size: 1.2rem; }
.nav-item.active { color: #FF4D00; }

.nav-cta-wrap {
  display: flex; align-items: center; justify-content: center;
}
.nav-cta {
  width: 54px; height: 54px; background: #FF4D00; border: none;
  display: flex; align-items: center; justify-content: center;
...
.nav-cta:active { background: #CC3D00; transform: scale(0.95); }
